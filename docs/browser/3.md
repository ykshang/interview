...
4. **需要服务器配置支持**：由于 URL 不含`#`，在页面刷新或者初次加载时，服务器会将其识别为对应路径的静态资源，去请求对应的资源。因此需要服务器配置路由表，如果找不到对应资源，就根据对应的路由返回对应的归属页面。也可以使用一些中间件 `connect-history-api-fallback` 在node服务中处理。
...
**其核心原理相当于：**
1. Router路由
   1. Router(路由):实现一个路由表，用于注册和存放路由信息和对应的视图信息
   2. 对外暴露操作路由的方法：诸如前进，后退，压入，推出，跳转等方法
   3. 视图刷新方法：路由发生改变后，根据新路由，拿到对应的视图信息，渲染到容器里

2. View部分：用于加载根据路由拿到的组件信息。

**特点：**
1. 依赖Node.js环境，不依赖于浏览器
2. 不支持普通web应用，适用于服务端渲染、小程序、单元测试、桌面应用等不涉及浏览器和URl的场景。
3. 不需要服务器配置
